...
async fn test_card_with_title_creation() {
    ...
    assert_eq!(body["data"]["zettel_id"].as_str().unwrap(), "REVIEW-TEST-001");
}
...
async fn test_card_without_title_creation() {
    ...
    assert_eq!(body["data"]["zettel_id"].as_str().unwrap(), "REVIEW-TEST-002");
}
...
async fn test_card_list_includes_titles() {
    ...
    // Create the cards
    server.post("/api/cards").json(&card1).await.assert_status_ok();
    server.post("/api/cards").json(&card2).await.assert_status_ok();
    ...
    let has_title_card = if first_card["title"].is_string() { first_card } else { second_card };
    let no_title_card = if first_card["title"].is_null() { first_card } else { second_card };
...
async fn test_review_card_data_structure() {
    ...
    assert_eq!(card_data["zettel_id"].as_str().unwrap(), "REVIEW-STRUCT-001");
    assert!(card_data["content"].as_str().unwrap().contains("Review Content"));
...
async fn test_collapsible_card_content_structure() {
    ...
    let test_cases = vec![
        ...
        ("COLLAPSE-001", Some("Short Title"), "This is the full content that should be collapsible"),
        // Card without title - should show content preview
        ("COLLAPSE-002", None, "This is content without a title that should still be collapsible"),
        // Card with long title
        ("COLLAPSE-003", Some("This is a very long title that might affect the collapsible display behavior"), "Short content"),
    ];
...
async fn test_review_interface_title_priority() {
    ...
    assert!(card_data["content"].as_str().unwrap().contains("This Markdown Header"));
    
    // The review interface should use the title field, not extract from content
    assert_ne!(card_data["title"].as_str().unwrap(), "This Markdown Header Should Not Override Title");
}
...
async fn test_expand_collapse_content_lengths() {
    ...
    let content_cases = vec![
        ("Very short", "SHORT"),
        ("Medium length content that spans multiple lines and contains enough text to warrant collapsing in the review interface", "MEDIUM"),
        ("Very long content that definitely needs to be collapsed by default. This content is intentionally long to test the collapsible interface behavior when dealing with large amounts of text that exceed normal display limits.", "LONG"),
    ];
    ...
    for (i, (content, case_type)) in content_cases.iter().enumerate() {
        ...
        assert_eq!(card_data["title"].as_str().unwrap(), format!("Title for {} Content", case_type));

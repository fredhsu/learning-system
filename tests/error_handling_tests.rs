use learning_system::{CardService, CreateCardRequest, Database, UpdateCardRequest};
use uuid::Uuid;
...
async fn test_invalid_uuid_handling() {
    ...
    for invalid_id in invalid_ids {
        ...
        let result = service
            .update_card(invalid_id, update_request)
            .await
            .unwrap();
        assert!(result.is_none());
...
async fn test_topic_name_constraints() {
    ...
    let topic1 = service
        .create_topic("Normal Topic".to_string(), None)
        .await
        .unwrap();

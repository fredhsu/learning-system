...
async fn apply_updates(db: &Database, updates: &[CardUpdate]) -> Result<MigrationStats> {
    ...
    for update in updates {
        match sqlx::query("UPDATE cards SET title = ?1, content = ?2 WHERE id = ?3")
            ...
            .await
        {
            Ok(result) => {
                if result.rows_affected() > 0 {
                    ...
                    println!(
                        "✓ Updated card {} with title: \"{}\"",
                        update.zettel_id, update.extracted_title
                    );
                } else {
                    stats
                        .errors
                        .push(format!("No rows affected for card {}", update.zettel_id));
                }
            }
            Err(e) => {
                stats
                    .errors
                    .push(format!("Failed to update card {}: {}", update.zettel_id, e));
            }
        }
    }
...
async fn main() -> Result<()> {
    ...
    // Connect to database
    let database_url =
        env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite:./learning_system.db".to_string());
    ...
    for (i, update) in updates.iter().enumerate() {
        if i < 5 || dry_run {
            // Show first 5 in live mode, all in dry-run

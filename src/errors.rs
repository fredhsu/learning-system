        ...
        match &self {
            ...
            ApiError::LLMError(_) => {
                ...
                (
                    ...
                    Json(ApiResponse::error("AI service temporarily unavailable. Please try again.".to_string()))
                )
            }
            ApiError::DatabaseError(_) => {
                ...
                (
                    ...
                    Json(ApiResponse::error("Database operation failed. Please try again.".to_string()))
                )
            }
            ApiError::InternalError(_) => {
                ...
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(ApiResponse::error("An internal error occurred. Please try again.".to_string()))
                )
            }
        }
    }
...
macro_rules! api_error {
    ...
    (validation, $operation:expr, $resource_type:expr, $message:expr) => {
        crate::errors::ApiError::ValidationError($message.to_string())
            .to_response_with_context(
                crate::errors::ErrorContext::new($operation, $resource_type)
            )
    };
    
    (duplicate, $operation:expr, $resource_type:expr, $id:expr) => {
        crate::errors::ApiError::DuplicateResource(format!("{} with id '{}' already exists", $resource_type, $id))
            .to_response_with_context(
                crate::errors::ErrorContext::new($operation, $resource_type).with_id($id)
            )
    };
    ...
    if error_str.contains("already exists") || error_str.contains("unique constraint") {
        ...
        if let Some(start) = error_str.find("'") {
            if let Some(end) = error_str[start + 1..].find("'") {
                ...
                return ApiError::DuplicateResource(format!("Resource '{}' already exists", identifier));
            }
        }
...
mod tests {
    ...
    fn test_error_context_creation() {
        ...
        assert_eq!(context.user_friendly_message, Some("Custom message".to_string()));
    }

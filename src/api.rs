...
pub async fn create_card(
    ...
    match state
        .card_service
        .create_card_with_zettel_links(request.clone())
        .await
    {
        Ok(card) => {
            info!(
        ...
        Err(e) => {
            ...
            let context = ErrorContext::new("create_card", "card").with_id(&request.zettel_id);
    ...
    match state.card_service.get_card(id).await {
        ...
        Err(e) => {
            log_api_error!(
                ...
                card_id = id,
                error = e,
                "database error retrieving card"
            );
...
pub async fn get_card_by_zettel_id(
    ...
    match state.card_service.get_card_by_zettel_id(&zettel_id).await {
        ...
        Ok(None) => {
            let error =
                ApiError::NotFound(format!("Card with Zettel ID '{}' not found", zettel_id));
            let context = ErrorContext::new("get_card_by_zettel_id", "card").with_id(&zettel_id);
            Err(error.to_response_with_context(context))
        }
        Err(e) => {
            let error = ApiError::DatabaseError(e);
...
pub async fn update_card(
    ...
    match state
        .card_service
        .update_card_with_zettel_links(id, request)
        .await
    {
        Ok(Some(card)) => {
            info!(
...
pub async fn create_topic(
    ...
    match state
        .card_service
        .create_topic(name.to_string(), description)
        .await
    {
...
async fn get_due_cards_for_session(
    card_service: &CardService,
) -> Result<Vec<Card>, (StatusCode, Json<ApiResponse<()>>)> {
...
async fn generate_session_questions(
    ...
    info!(
        ...
        "Starting question generation for review session"
    );
...
pub async fn submit_session_answer(
    ...
    match state
        .llm_service
        .grade_answer(&card, question, &request.answer)
        .await
    {
        Ok(grading_result) => {
            info!(
...
pub async fn submit_batch_session_answers(
    ...
    let batch_requests: Vec<BatchGradingRequest> = request
        .answers
        .iter()
        .map(|answer_request| {
...
pub async fn submit_parallel_session_answers(
    ...
    let questions_and_answers: Vec<(QuizQuestion, String)> = request
        .answers
        .iter()
        .map(|answer_request| {
            let question = &questions[answer_request.question_index];
        ...
        .await
    {
        Ok((grading_results, mode_used, fallback_reason)) => {
            ...
            let metrics = ParallelProcessingMetrics {
                ...
                concurrent_execution_count: request
                    .max_concurrent_tasks
                    .unwrap_or(5)
                    .min(grading_results.len()),
                average_task_duration_ms: total_duration.as_millis() as u64
                    / grading_results.len().max(1) as u64,
...
pub async fn submit_quiz_answer(
    ...
    match state
        .llm_service
        .grade_answer(&card, &dummy_question, &request.answer)
        .await
    {
        Ok(grading_result) => {
            info!(
            ...
            match state
                .card_service
                .review_card(card_id, grading_result.suggested_rating)
                .await
            {
...
pub async fn review_card(
    ...
    match state
        .card_service
        .review_card(card_id, request.rating)
        .await
    {
...
pub fn create_router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            "/api/review/session/:session_id/answer/:card_id",
            post(submit_session_answer),
        )
        .route(
            "/api/review/session/:session_id/answers/:card_id/batch",
            post(submit_batch_session_answers),
        )
        .route(
            "/api/review/session/:session_id/answers/:card_id/parallel",
            post(submit_parallel_session_answers),
        )
        // Review routes
